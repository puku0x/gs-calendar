...
public class CalendarView extends AbstractCalendarView implements View.OnClickListener {
    ...
    @Override
    public LinearLayout getCategoryViewForDate(final long dayInMillis) {
        // Loop through all children
        final int childCount = getChildCount();
        for(int i = 0; i < childCount; i++) {
            final View weekLayout = getChildAt(i);
            if(weekLayout != null) {
                ...
                if(dayLayout != null) {
                    // Find the TextView, and return it
                    if (dayLayout.isEnabled()) {
                        return (LinearLayout) dayLayout.findViewById(R.id.lib_calendar_day_categories);
                    }
                }
            }
        }
